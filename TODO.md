...
- [x] Google Calendar/Gmail OAuth connection **dropped by user request** — hit Google's "unverified app" test-user wall mid-flow and decided not to chase it further. The code stays in the repo (harmless, no-ops when unconfigured, exactly as it did before this was attempted), but nobody should expect it to be live-connected. Manual email/WhatsApp import and generic IMAP remain the fully-working message sources.
- [x] Along the way, fixed `scripts/connect-google.js`'s redirect URI to match what Desktop-app OAuth clients actually register (bare `http://localhost`, not `/oauth2callback`), confirmed against a real downloaded `client_secret.json`. Also added auto-detection of a `client_secret*.json` dropped in the project root (skips manual Client ID/Secret copy-paste) and added it to `.gitignore` so it can never get committed by accident.
- [x] **Work/personal split for People** — `localSchema.js` contacts gained a `category` field (`"work"` | `"personal"`, defaults to `"personal"` for every auto-created contact). `components/PeoplePanel.jsx` now shows two clear sections (Work / Personal) instead of one flat list, each with its own "+ Add" and independent counts; the add/edit form has a two-button Work/Personal toggle plus the existing vip/casual/mute priority dropdown. Verified end-to-end via curl: explicit work-category contact, explicit personal-category contact, and an auto-created contact (defaults to personal) all landed in the right section with the right data.
